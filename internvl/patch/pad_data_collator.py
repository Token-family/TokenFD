...
def concat_pad_data_collator(features, pad_id=0):
    ...
    # Again, we will use the first element to figure out which key/values are not None for this model.
    for k, v in first.items():
        if k not in ('label', 'label_ids', 'pixel_values', 'image_flags', 'image_flags_ori', 'mask_values', 'masks_flags') and \
            ...
            if isinstance(v, torch.Tensor):
                ...
                batch[k] = torch.tensor([f[k] for f in features])
        if k in ('pixel_values', 'image_flags', 'image_flags_ori', 'mask_values', 'masks_flags'):
            if isinstance(v, torch.Tensor):
