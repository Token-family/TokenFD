...
from tools.random_generate_bbox import generate_mask, generate_random_bboxes, draw_bboxes, generate_mask_multivalue, convert
...
class ModelArguments:
    ...
    ocr_mlp_path: Optional[str] = field(
        default=None,
...
class DataTrainingArguments:
    ...
    rewrite_temp_json: bool = field(
        default=False,
        metadata={'help': 'need to rewrite temp json when repeite time changes. Default is False.'},
    )
    use_background_learning: bool = field(
        default=False,
        metadata={'help': 'need to rewrite temp json when repeite time changes. Default is False.'},
    )
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        self.use_background_learning = use_background_learning
        logger.info(f'[Dataset] num_image_token: {num_image_token}')
        logger.info(f'[Dataset] dynamic_image_size: {dynamic_image_size}')
        logger.info(f'[Dataset] use_thumbnail: {use_thumbnail}')
        logger.info(f'[Dataset] min_dynamic_patch: {min_dynamic_patch}, max_dynamic_patch: {max_dynamic_patch}')
        logger.info(f'[Dataset] use_background_learning: {use_background_learning}')
        ...
        data_dir = os.path.join(os.path.dirname(meta['annotation']), f'{basename}_temp_seg')
        ...
        if os.path.exists(temp_path) and not rewrite_temp_json:
            # If it exists, read the raw data from the file
            with open(temp_path, 'r') as f:
                self.raw_data = f.readlines()
        else:
            logger.info(f"New_add: rewrite {meta['annotation']} to temp_file for {repeat_time} times")
        ...
        self.root = meta['root']
        try:
            self.replace_root = meta['replace_root']
        except:
            self.replace_root = ''
        try:
            self.npy_path = meta['npy_path']
        except:
            self.npy_path = None
        try:
            # import pdb; pdb.set_trace()
            self.replace_bb = meta['replace_bb']
        except:
            self.replace_bb = ''
    ...
    def load_image(self, image_path):
        ...
        if self.txn is not None:
            # print(image_path)
            image_key = image_path.split('/')[-1].encode()
            imgbuf = self.txn.get(image_key)
            buf = six.BytesIO()
            buf.write(imgbuf)
            buf.seek(0)

            if self.use_background_learning:
                # import pdb; pdb.set_trace()
                current_index = image_path.split('/')[-1].replace('image-', '')
                label_key = f"gtWord-{current_index}".encode()
                label = self.txn.get(label_key).decode("utf-8")
                
                label_path = os.path.join(f'{self.root}/train/full_image', self.ds_name, '/'.join(label.split('/')[-2:]))
                bboxes = []
                with open(label_path, 'r') as f:
                    ...
                    for line in lines:
                        if line == '\n':
                            continue
                        word_pts = line.split('\t')[0].split(',')
                        word_pts = [float(pts) for pts in word_pts]
                        polygon = np.array(word_pts).astype(np.float_)
                        if len(polygon) != 8:
                            Ploygon_r = Polygon(polygon.reshape(-1,2))
                            min_rect = Ploygon_r.minimum_rotated_rectangle
                            # 输出最小外接矩形的坐标
                            try:
                                polygon_r = np.array(list(min_rect.exterior.coords)).reshape(-1,2)   
                            except:
                                continue 
                            polygon_r = polygon_r[:4].reshape(4, 2)         
                        elif len(polygon) == 8:
                            polygon_r = polygon.reshape(4, 2)
                        bboxes.append(polygon_r)

                return Image.open(buf), np.array(bboxes).reshape(-1, 4, 2)

            return Image.open(buf)
        else:
            if self.use_background_learning:
                current_root = os.path.join(self.replace_bb, self.ds_name)
                txt_path = current_root + image_path.split(f'/{self.ds_name}')[1][:-4]+'.txt'
                with open(txt_path, 'r') as f:
                   bboxes = f.read()
                   bboxes = eval(bboxes)
                return Image.open(image_path).convert('RGB'), np.array(bboxes)
    ...
    def get_image_path(self, image_path):
        if image_path.startswith('s3://'):  # for ceph
            ...
            if self.replace_root != '':  
                image_path = image_path.replace(self.replace_root, '')
                image_path = self.root + image_path
            else:
                image_path = self.root + image_path
        return image_path
    ...
    def gen_mask_item(self, data_item, limit_num=30, mask=None):
        selected_token = [2]*limit_num
        if self.ds_name != 'laion-ocr':
            bpe_mask = torch.load(os.path.join(self.npy_path, data_item['npy_path']), map_location="cpu")
        else:
            main_number_str = data_item['npy_path'].split('/')[-2]
            file_name = data_item['npy_path'].split('/')[-1]
            main_number = int(main_number_str)
            number = int(data_item['npy_path'].split('/')[-1].replace('image-','').replace('.pt',''))
            sub_dir = (number - main_number*1000000) // 50000 + 1
            bpe_path = os.path.join(self.npy_path, f'npy_result/laion-ocr/{main_number_str}/%03d/{file_name}' % (sub_dir))
            bpe_mask = torch.load(bpe_path, map_location="cpu")

        bpe_infoes = data_item['bpe_json_info']
        
        gt = torch.zeros((limit_num, bpe_mask.shape[0], bpe_mask.shape[1]),dtype=torch.uint8)

        if mask is not None:
            background_sample_numbers = mask.max()
            for bpe_index in range(background_sample_numbers):
                selected_token[bpe_index] = self.tokenizer(' ')['input_ids'][1]
                # import pdb; pdb.set_trace()
                gt[bpe_index, mask == bpe_index+1] = 1
        else:
            background_sample_numbers = 0

        rest_num = limit_num - background_sample_numbers

        if len(bpe_infoes) >= rest_num:
            bpe_infoes = random.sample(bpe_infoes, rest_num)
            bpe_length = rest_num + background_sample_numbers
        else:
            bpe_length = len(bpe_infoes) + background_sample_numbers
            pad_length = limit_num - bpe_length
        
        for bpe_index, bpe_info in enumerate(bpe_infoes):
            selected_token[background_sample_numbers+bpe_index] = self.tokenizer(bpe_info['bpe'])['input_ids'][1]
            gt[background_sample_numbers+bpe_index, bpe_mask == bpe_info['bpe_mask_index']] = 1

        return gt, selected_token, bpe_length
    
    def gen_mask_item_qw(self, data_item, limit_num=30, mask=None):
        def concat_bpe(bpe_json_info):
            def get_model_bpe(tokenizer, word):

                res = []
                for item in tokenizer.encode(word):
            ...
            for item in bpe_json_info:

                if cur_sub_word is None or cur_sub_word != item['word']:
                    cur_sub_word = item['word']
                    model_bpe_res = get_model_bpe(self.tokenizer, cur_sub_word)
                    visit_ = [0] * len(model_bpe_res)
                    tmp_items = []
                    tmp_str = ''

                json_bpe = item['bpe']
        
                if json_bpe.startswith(' '):
                    json_bpe = json_bpe[1:]

                for idx, cur_bpe in enumerate(model_bpe_res):
                    if visit_[idx] == 1:
                        continue

                    if json_bpe == cur_bpe:
                        final_bpe_info.append(item)
                        visit_[idx] = 1
                        break

                    elif json_bpe in cur_bpe:
                        tmp_str += json_bpe
                        tmp_items.append(item)
                        
                        if len(tmp_str) >= len(cur_bpe):
                            if tmp_str == cur_bpe:
                                final_bpe_info.append({
                                    'word': item['word'], 
                                    'bpe': ''.join([pos['bpe'] for pos in tmp_items]), 
                                    'bpe_mask_index': [pos['bpe_mask_index'] for pos in tmp_items], 
                                    'bpe_text_index': [pos['bpe_text_index'] for pos in tmp_items]
                                })
                                visit_[idx] = 1
        ...
        bpe_infoes = data_item['bpe_json_info']
        # import pdb; pdb.set_trace()
        bpe_infoes = concat_bpe(bpe_infoes)
        ...
        for bpe_index, bpe_info in enumerate(bpe_infoes):
            ...
            selected_token[background_sample_numbers+bpe_index] = self.tokenizer(bpe_info['bpe'])['input_ids'][0]
            bpe_mask_indexs = bpe_info['bpe_mask_index']
            if isinstance(bpe_mask_indexs, list) == False:
                bpe_mask_indexs = [bpe_mask_indexs]
            for cur_index in bpe_mask_indexs:
                gt[background_sample_numbers+bpe_index, bpe_mask == cur_index] = 1
    ...
    def multi_modal_get_item(self, data_item):
        ...
        # Load the image using tcs_loader if available, otherwise use PIL
        if self.use_background_learning:
            image, bboxes = self.load_image(image_path)
            bboxes_map = generate_mask(image, bboxes)
            background_bboxes = generate_random_bboxes(bboxes_map, num_bboxes=10, min_size=10, max_size=20)
            
            background_bboxes = convert(background_bboxes, order='xyxy2quad')
            background_masks = generate_mask_multivalue(image, background_bboxes)
        else:
            image = self.load_image(image_path)
            background_masks = None
        limit_nums = 70 
        mask, selected_tokens, num_tokens = self.gen_mask_item_qw(data_item, limit_num=limit_nums, mask=background_masks)

        if self.dynamic_image_size:  # If dynamic image size is enabled, preprocess the image dynamically
            images = dynamic_preprocess(image, min_num=self.min_dynamic_patch, max_num=self.max_dynamic_patch,
                                        image_size=self.image_size, use_thumbnail=self.use_thumbnail)
            masks = dynamic_preprocess_mask(mask, min_num=self.min_dynamic_patch, max_num=self.max_dynamic_patch,
                                        image_size=self.image_size, use_thumbnail=self.use_thumbnail)
        ...
        num_mask_patches = masks.size(0)
        assert num_patches == num_mask_patches
        ...
        ret = dict(
            ...
            pixel_values=pixel_values, #(13, 448, 448)
            mask_values = masks, #(13, 30, 448, 448)
            masks_flags=torch.concatenate([torch.tensor([1] * num_tokens, dtype=torch.long), torch.tensor([0] * (limit_nums - num_tokens), dtype=torch.long)]), #(<30)
            image_flags=torch.tensor([num_patches], dtype=torch.long), #(1)
            mask_nums=torch.tensor([num_mask_patches], dtype=torch.long)
    ...
    def open_lmdb(self):
        lmdb_path = os.path.join(self.root, 'train/full_image', self.meta['name'], 'lmdb')
        print(lmdb_path)
        self.env = lmdb.open(lmdb_path, max_readers=1, readonly=True, create=False,
...
def build_datasets(
    ...
    try:
        ...
        print(f'{data_args.meta_path} error!!!!!!!!!!!')
    for ds_idx, ds_name in enumerate(ds_collections.keys()):
        ...
        ds_collections[ds_name]['name'] = ds_name
        dataset = LazySupervisedDataset(
            ...
            rewrite_temp_json=data_args.rewrite_temp_json,
            ...
            random_seed=ds_idx,
            use_background_learning=data_args.use_background_learning,
        )
...
def main():
    ...
    if model_args.model_name_or_path is not None:
        ...
        internvl_chat_config = InternVLChatConfig(
            ...
            train_stage=model_args.train_stage)
    ...
    if model_args.ocr_mlp_path is not None:
        logger.info('Loading pretrained OCR_MLP projector...')
        state_dict = torch.load(model_args.mlp_path, map_location='cpu')
        message = model.ocr_mlp.load_state_dict(state_dict)
        logger.info(message)
