...
class ModelArguments:
    ...
    vision_mt_path: Optional[str] = field(
        default=None,
        metadata={'help': 'Path to pretrained model or model identifier from huggingface.co/models'}
    )
    reducer_path: Optional[str] = field(
        default=None,
        metadata={'help': 'Path to pretrained model or model identifier from huggingface.co/models'}
    )
    ...
    freeze_mlp_mt: bool = field(
        default=False,
        metadata={'help': 'Set to True to freeze the MLP. Default is False.'},
    )
    freeze_reducer: bool = field(
        default=False,
        metadata={'help': 'Set to True to freeze the MLP. Default is False.'},
    )
    freeze_backbone_mt: bool = field(
        default=False,
        metadata={'help': 'Set to True to freeze the MLP. Default is False.'},
    )
    ...
    token_level: bool = field(
        default=False,
        metadata={'help': 'Set to True to enable the use of token level.'},
    )
...
class LazySupervisedDataset(Dataset):
    ...
    def gen_mask_item(self, data_item, limit_num=30):
        ...
        for _, bpe_info in enumerate(bpe_infoes):
            
            if bpe_info['bpe_text_index'] - 1 + 1900 >= 4095:
                continue
            
            new_bpe_infoes.append(bpe_info)
        
        bpe_infoes = new_bpe_infoes
        
        selected_token_index = []
        
        gt = torch.zeros((limit_num, bpe_mask.shape[0], bpe_mask.shape[1]),dtype=torch.uint8)

        if len(bpe_infoes) >= limit_num:
            bpe_infoes = random.sample(bpe_infoes, limit_num)
            bpe_length = limit_num
        else:
            bpe_length = len(bpe_infoes)
        ...
        for bpe_index, bpe_info in enumerate(bpe_infoes):
            ...
            selected_token[bpe_index] = self.tokenizer(bpe_info['bpe'])['input_ids'][1]
            
            selected_token_index.append(bpe_info['bpe_text_index'] - 1)
            gt[bpe_index, bpe_mask == bpe_info['bpe_mask_index']] = 1
            i += 1

       
        assert i == bpe_length
                
        return gt, selected_token, bpe_length, selected_token_index
    ...
    def multi_modal_get_item(self, data_item):
        ...
        if self.token_level:
            limit_nums = self.limit_num #30
            mask, selected_tokens, num_tokens, selected_token_index = self.gen_mask_item(data_item, limit_num=limit_nums)
        ...
        ori_label = data_item['conversations'][1]['value']
        
        if data_item['conversations'][1]['value'].startswith(' '):
            data_item['conversations'][1]['value'] = data_item['conversations'][1]['value'][1:]
        ...
        ret = dict(
            ...
            image_flags_ori=torch.tensor([1] * num_patches, dtype=torch.long)
        )

        if self.token_level:
            
            if all(element == 2 for element in selected_tokens):
                raise 'no vaild bpe, skip'

            ret['mask_values'] = masks #(13, 30, 448, 448)
            ret['masks_flags'] = torch.concatenate([torch.tensor([1] * num_tokens, dtype=torch.long), torch.tensor([0] * (limit_nums - num_tokens), dtype=torch.long)]) #(<30)
            ret['image_flags'] = torch.tensor([num_patches], dtype=torch.long) #(1)
            ret['mask_nums'] = torch.tensor([num_mask_patches], dtype=torch.long)
            
            ret['select_token_input_id'] = selected_tokens

            label_i = (ret['labels'] != -100).nonzero(as_tuple=False)[0][0]  #1845
                
            ret['selected_token_index'] = [item + label_i.item() for item in selected_token_index]

            if len(ret['selected_token_index']) < self.limit_num:
                for num in range(self.limit_num - len(ret['selected_token_index'])):
                    ret['selected_token_index'].append(len(ret['input_ids']) - 1 - num)
            
            
            if max(ret['selected_token_index']) >= len(ret['input_ids']) or min(ret['selected_token_index']) < 0:
                raise 'bpe index out of range'
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        # print('root:{:}'.format(os.path.join(self.root, self.meta['name'])))
...
def build_datasets(
    ...
    for ds_idx, ds_name in enumerate(ds_collections.keys()):
        ...
        dataset = LazySupervisedDataset(
            ...
            random_seed=ds_idx,
            token_level=data_args.token_level,
        )
...
def main():
    ...
    data_args.token_level = model_args.token_level
    ...
    if model_args.model_name_or_path is not None:
        ...
        config.train_stage=model_args.train_stage
        config.token_level=model_args.token_level
    ...
    if model_args.vision_mt_path is not None:
        logger.info('Loading pretrained marten encoder...')
        state_dict = torch.load(model_args.vision_mt_path, map_location='cpu')
        from internvl.model.internvl_chat.modeling_marten_encoder import change_state_dict
        state_dict = change_state_dict(state_dict)
        message = model.encoder_mt.load_state_dict(state_dict)
        logger.info(message)

    if model_args.reducer_path is not None:
        logger.info('Loading pretrained reducer...')
        state_dict = torch.load(model_args.reducer_path, map_location='cpu')
        new_st = {}
        for k, v in state_dict.items():
            new_st[k.replace('model.vision_tower.vision_tower_extra_token_reducer.', '')] = v
        state_dict = new_st
        message = model.reducer_mt.load_state_dict(state_dict)
        logger.info(message)
    ...
    if model_args.freeze_mlp_mt:
        _freeze_params(model.mlp_mt)
    
    if model_args.freeze_reducer:
        _freeze_params(model.reducer_mt)

    if model_args.freeze_backbone_mt:
        _freeze_params(model.encoder_mt)
